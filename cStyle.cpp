...
// implimantation file
// protected methods
bool cStyle::isFilename() const
{
	return filename[0] != '\0';
}

void cStyle::extract() // still no need of bool return in this fun
{
...
void cStyle::addFilename(const std::string &str)
{
	filename = str;
}
void cStyle::save(cStyle::state s)
{
	std::ofstream out;
	std:: ifstream input;
	...
	switch (s)
	{
		case original:
		mytag = "cStyle_org_";
		nfile.insert(0,mytag);
		out.open(nfile.c_str());
		if(!out.is_open())
		{
